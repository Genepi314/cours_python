...
class PongBall(PongSprite):
    ...
    # On aurait pu ausi coller plus au code de départ.
    # voir pong.py :
    # On reprend le choice[-1, 1], mais on demande
    # à Python de faire un uniform() et de choisir 
    # au hasard une décimale comprise dans cet ambitus.
    # Cela correspond à une direction pour un 
    # vecteur normalisé!!

        angle_au_hasard1 = randint(45, 135)
        angle_au_hasard2 = randint(225, 315)
        angles_au_hasard = [angle_au_hasard1, angle_au_hasard2]
        angle_vrai = choice(angles_au_hasard)

        self.position = self.start_position
        self.speed = 200, 200
        speed_x, speed_y = self.speed

        angle = radians(-angle_vrai + 90)
